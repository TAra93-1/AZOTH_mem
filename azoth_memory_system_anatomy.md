...
```mermaid
flowchart LR
    A["输入<br/>对话 · 手动记录 · 文件 · 笔记"] --> B["提取<br/>判断什么值得长期保留"]
...
每次更新应读取旧画像，只对必要部分做增删改，并保存写前快照与支持来源。画像无法安全识别时，宁可暂停，也不要用模型输出覆盖原文。
